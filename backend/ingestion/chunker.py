...
class ParentIDGenerator:
    """
    Generate parent_id from section title.
    Extracts only the subsection number (e.g., "2.0" from "2.0 Fundamentals...").
    """
    
    @staticmethod
    def extract_subsection(title: str) -> str:
        """
        Extract subsection number from title.
        
        Examples:
            "2.0 Fundamentals of Systems Engineering" → "2.0"
            "3.5 Risk Management Analysis" → "3.5"
            "4.2.1 Detailed Process" → "4.2.1"
        ...
        Returns:
            str: Subsection number (e.g., "2.0"), or "unknown" if not found
        """
        if not title:
            return "unknown"
        
        # Match pattern at the start: digits, dots, digits (e.g., "2.0", "3.5", "4.2.1")
    ...
    def sanitize_title_to_id(title: str) -> str:
        """
        Convert section title to parent_id using only the subsection number.
        
        Examples:
            "2.0 Fundamentals of Systems Engineering" → "section_2_0"
            "3.5 Risk Management Analysis" → "section_3_5"
            "4.2.1 Detailed Process" → "section_4_2_1"
        
        Args:
            title (str): The section title
        
        Returns:
            str: Sanitized parent_id based on subsection only
        """
        if not title:
            return "section_unknown"
        
        # Extract subsection number
        subsection = ParentIDGenerator.extract_subsection(title)
        
        # Replace dots with underscores (e.g., "2.0" → "2_0")
        sanitized = subsection.replace(".", "_")
        
        # Lowercase (redundant but safe)
        sanitized = sanitized.lower()
        
        return f"section_{sanitized}"
...
def create_chunks(elements, section, max_tokens=MAX_TOKENS, overlap_pct=OVERLAP_PCT):
    ...
    Returns:
        list: List of chunk dicts with text, tables, images, page info, and parent metadata
    """
    chunks = []
    overlap_tokens = int(max_tokens * overlap_pct)
    
    # Generate parent_id from section title
    parent_id = ParentIDGenerator.sanitize_title_to_id(section["title"])
...
def create_chunk_from_buffer(buffer_text, start_page, end_page, tables, images, used_table_indices, used_image_indices, chunk_id, section, parent_id):
    """
    ...
    Args:
        ...
        section (dict): Section metadata (title, page range)
        parent_id (str): Parent section identifier
    
    Returns:
        dict: Chunk with text, tables, images, page info, parent metadata, and references
    """
    chunk_tables = []
    chunk_images = []
    ...
    return {
        ...
        "parent_id": parent_id,  # ✅ Parent section identifier
        "parent_title": section["title"],  # ✅ Parent section title
        "section_from_page": section["from_page"],  # ✅ Parent section start page
        "section_to_page": section["to_page"]  # ✅ Parent section end page
    }
    ...
    for chunk in chunks:
        ...
        docs.append(Document(
            ...
            metadata={
                ...
                "parent_from_page": chunk["section_from_page"],
                "parent_to_page": chunk["section_to_page"]
            }
        ))
...
# =============== PARENT INDEX BUILDER ===============
def build_parent_index_from_documents(all_docs):
    """
    Build a parent index by grouping documents by their parent_id.
    This creates a separate lookup structure for retrieving full parent content.
    
    Args:
        all_docs (list): All Document objects with parent_id metadata
    
    Returns:
        dict: Parent index mapping parent_id to parent content and metadata
    """
    parent_groups = defaultdict(lambda: {
        ...
        "content": "",
        "tokens": 0,
        "page_range": {"start": float('inf'), "end": 0},
        "types": set(),
        "chunk_ids": set()
    })
    ...
    for doc in all_docs:
        ...
        # Add document to parent group
        parent_groups[parent_id]["title"] = parent_title
        parent_groups[parent_id]["docs"].append(doc)
        parent_groups[parent_id]["content"] += "\n\n" + doc.page_content
        parent_groups[parent_id]["tokens"] += doc.metadata.get("token_count", len(doc.page_content.split()))
        
        # Track page range
        parent_groups[parent_id]["page_range"]["start"] = min(
            parent_groups[parent_id]["page_range"]["start"], page_start
        )
        parent_groups[parent_id]["page_range"]["end"] = max(
            parent_groups[parent_id]["page_range"]["end"], page_end
        )
        
        # Track types
        parent_groups[parent_id]["types"].add(chunk_type)
        
        # Track chunk IDs
        parent_groups[parent_id]["chunk_ids"].add(chunk_id)
    
    # Convert sets to lists and create final index
    parent_index = {}
    for parent_id, parent_data in parent_groups.items():
        parent_index[parent_id] = {
            "parent_id": parent_id,
            "title": parent_data["title"],
            "docs": parent_data["docs"],  # List of Document objects
            "content": parent_data["content"].strip(),
            "tokens": parent_data["tokens"],
            "page_range": {
                "start": int(parent_data["page_range"]["start"]),
                "end": int(parent_data["page_range"]["end"])
            },
            "types": list(parent_data["types"]),
            "chunk_ids": list(parent_data["chunk_ids"]),
            "doc_count": len(parent_data["docs"])
        }
...
def process_pdf(pdf_path, noise_vars=None, max_tokens=MAX_TOKENS, overlap_pct=OVERLAP_PCT):
    ...
    Returns:
        tuple: (all_docs, parent_index) where:
            - all_docs: List of Document objects
            - parent_index: Dictionary mapping parent_id to parent metadata
    """
    if noise_vars is None:
        noise_vars = NOISE_VARIABLES
    ...
    for section in sections:
        ...
        # ✅ Use updated create_chunks with parent metadata
    ...
    # ✅ BUILD PARENT INDEX
    print(f"\n🔗 Building parent index from {len(all_docs)} documents...")
    parent_index = build_parent_index_from_documents(all_docs)
    print(f"   ✓ Created parent index with {len(parent_index)} sections")
    
    for parent_id, parent_data in parent_index.items():
        print(f"     • {parent_data['title']}: {parent_data['doc_count']} docs, {parent_data['tokens']} tokens, pages {parent_data['page_range']['start']}-{parent_data['page_range']['end']}")

    # Save output
    output_file = os.path.join(OUTPUT_DIR, "documents.json")
    parent_index_file = os.path.join(OUTPUT_DIR, "parent_index.json")
    ...
    # Save parent index (without Document objects)
    parent_index_serializable = {}
    for parent_id, parent_data in parent_index.items():
        parent_index_serializable[parent_id] = {
            k: v for k, v in parent_data.items() 
            if k != "docs"  # Exclude Document objects
        }
    
    with open(parent_index_file, "w", encoding="utf-8") as f:
        json.dump(parent_index_serializable, f, indent=2, ensure_ascii=False)

    print(f"\n✅ Saved {len(all_docs)} documents from {total_chunks} chunks")
    print(f"📁 Documents: {output_file}")
    print(f"📁 Parent Index: {parent_index_file}\n")
    
    return all_docs, parent_index
...
if __name__ == "__main__":
    ...
    docs, parent_index = process_pdf(pdf_path)
    
    # Option 2: Use custom settings (uncomment to use)
    # docs, parent_index = process_pdf(
    #     pdf_path, 
    #     max_tokens=800,
    ...
    print(f"  Parent ID: {docs[0].metadata.get('parent_id')}")
    print(f"\nParent Index Example:")
    first_parent_id = list(parent_index.keys())[0] if parent_index else None
    if first_parent_id:
        print(f"  {first_parent_id}: {parent_index[first_parent_id]['title']}")
    print(f"\nTotal Documents: {len(docs)}")
    print(f"Total Parents: {len(parent_index)}")
